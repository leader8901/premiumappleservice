...
!function (factory) {
    ...
    window.scriptSysPayment = {}, window.handlerSysPayment = {}, window.tildaForm = {
        ...
        window.tildaForm.captchaCallback = function (token) {
            ...
            $jform = $($jform);
            var arError = [], isEmptyValue = !0;
            if ($jform.find(".js-tilda-rule").each((function () {
                var req = $(this).data("tilda-req") || 0, rule = $(this).data("tilda-rule") || "none", regExp, str,
                    domainpart, minlength = $(this).data("tilda-rule-minlength") || 0,
                    maxlength = $(this).data("tilda-rule-maxlength") || 0, error = {}, val = $(this).val(),
                    valnospace = "";
                ...
                if (val.length > 0 && (isEmptyValue = !1), minlength && (minlength = parseInt(minlength)), maxlength && (maxlength = parseInt(maxlength)), 1 == req && (0 == val.length && 0 == valnospace.length || ("checkbox" == $(this).attr("type") || "radio" == $(this).attr("type")) && 0 == $(this).closest("form").find('[name="' + $(this).attr("name") + '"]:checked').length)) error.type.push("req"); else {
                    switch (rule) {
                        case"email":
                ...
                var minOrderSetted = void 0 !== window.tcart_minorder && window.tcart_minorder > 0,
                    minQuantitySetted = void 0 !== window.tcart_mincntorder && window.tcart_mincntorder > 0, error,
                    error;
                if (minOrderSetted) if (window.tcart.prodamount >= window.tcart_minorder) ; else (error = {}).obj = $({}), error.type = [], error.type.push("minorder"), arError.push(error);
                if (minQuantitySetted) if (window.tcart.total >= window.tcart_mincntorder) ; else (error = {}).obj = $({}), error.type = [], error.type.push("minquantity"), arError.push(error)
            }
            ...
            for (var i = 0; i < arErrors.length; i++) if (arErrors[i] && arErrors[i].obj) {
            ...
            var clsInputBoxSelector = $jform.data("inputbox");
            clsInputBoxSelector || (clsInputBoxSelector = ".blockinput");
            for (var $fieldgroup, isFieldErrorBoxExist, isShowCommonErrors, $errItem, sError = "", i = 0; i < arErrors.length; i++) if (arErrors[i] && arErrors[i].obj) {
                if (0 == i && "none" == arErrors[i].obj) {
            ...
            if (-1 == event.origin.indexOf(window.tildaForm.endpoint)) ; else {
                if ("closeiframe" == event.data) return $("#tildaformcaptchabox").remove(), void $("#js-tildaspec-captcha").remove();
                var capthakey = event.data, $elem = $("#js-tildaspec-captcha");
            ...
            var virtPage = "/tilda/" + $jform.attr("id") + "/payment/",
                ...
                virtProduct = arArgs.description, currency;
            ...
            var companyname = arArgs.companyname, companylogo = arArgs.companylogo;
            ...
            var currency = arArgs.currency, language = arArgs.language, initCP = {};
            ...
            return $jpopup && 0 != $jpopup.length || ($jpopup = $jform.closest(".t706__cartwin_showed")), $jpopup.data("old-style", $jpopup.attr("style")), $jpopup.attr("style", "z-index:100"), window.tildaForm.orderIdForStat = arArgs.invoiceId, arArgs.skin || (arArgs.skin = "classic"), window.cloudpaymentshandler.charge({
                ...
                if ($jpopup.attr("style", $jpopup.data("old-style")), $jform.find(".js-successbox").hide(), $jform.data("successmessage") > "" ? $jform.find(".js-successbox").html($jform.data("successmessage")) : $jform.find(".js-successbox").html(""), $jform.data("successmessage", ""), window.cloudpaymentshandler = !1, arArgs.failureurl > "") window.location.href = arArgs.failureurl; else {
            ...
            var successcallback = $jform.data("success-callback");
            successcallback && successcallback.length > 0 && "t396_onSuccess" === successcallback && "function" == typeof window.t396_onSuccess ? eval(successcallback + "($jform)") : successcallback && successcallback.length > 0 && "t396_onSuccess" !== successcallback && eval(successcallback + "($jform)"), $jform.find(".t-upwidget-container__data_table_actions_remove svg").click(), $jform.find("input[type=text]:visible").val(""), $jform.find("textarea:visible").html(""), $jform.find("textarea:visible").val(""), $jform.data("tildaformresult", {
                tranid: "0",
                orderid: "0"
            }), $jform.trigger("tildaform:aftersuccess")
        }, window.tildaForm.banktransferPay = function ($jform, arArgs) {
            if (arArgs && "fast" == arArgs.condition) window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0); else if (arArgs && arArgs.html > "") {
            ...
            var html = "", $popup = $("#tildaformsuccesspopup");
            ...
            $jform.find(".js-successbox").length > 0 && ("" == $jform.find(".js-successbox").text() && t_form_dict("success") && $jform.find(".js-successbox").html(t_form_dict("success")), "y" == $jform.data("success-popup") ? window.tildaForm.showSuccessPopup($jform.find(".js-successbox").html()) : $jform.find(".js-successbox").show()), $jform.addClass("js-send-form-success"), successcallback && successcallback.length > 0 && "t396_onSuccess" === successcallback && "function" == typeof window.t396_onSuccess ? eval(successcallback + "($jform)") : successcallback && successcallback.length > 0 && "t396_onSuccess" !== successcallback ? eval(successcallback + "($jform)") : successurl && successurl.length > 0 && setTimeout((function () {
            ...
            if (2 == formtype || !formtype && formskey > "") {
                ...
                return $jform.trigger("tildaform:beforesend"), $.ajax({
                    ...
                    error: function (error) {
                        ...
                        $errBox && 0 != $errBox.length || ($jform.prepend('<div class="js-errorbox-all"></div>'), $errBox = $jform.find(".js-errorbox-all"));
                        var $allError = $errBox.find(".js-rule-error-all");
                        if ($allError && 0 != $allError.length || ($errBox.append('<p class="js-rule-error-all"></p>'), $allError = $errBox.find(".js-rule-error-all")), !error || 0 == error.status && ts_delta < 100) $allError.html("Request error (opening block content panel). Please check internet connection..."); else {
            ...
            if ("y" == $jform.data("is-formajax")) {
                ...
                return d = $jform.serializeArray(), window.tildaForm.tildapayment && window.tildaForm.tildapayment.amount && d.push({
                    ...
                    success: function (html) {
                        ...
                        if (btnformsubmit.removeClass("t-btn_sending"), btnformsubmit.data("form-sending-status", "0"), btnformsubmit.data("submitform", ""), html && html.length > 0) if ("{" == html.substring(0, 1)) {
                            if ((json = window.JSON && window.JSON.parse ? window.JSON.parse(html) : $.parseJSON(html)) && json.message) "OK" != json.message && $jform.find(".js-successbox").html(json.message); else if (json && json.error) {
                                var $errBox = $jform.find(".js-errorbox-all");
                                $errBox && 0 != $errBox.length || ($jform.prepend('<div class="js-errorbox-all"></div>'), $errBox = $jform.find(".js-errorbox-all"));
                                var $allError = $errBox.find(".js-rule-error-all");
            ...
            $(".js-tilda-mask").each((function () {
                var mask = $(this).data("tilda-mask"), maskplaceholder = $(this).data("tilda-mask-holder");
        ...
        $jallforms.length > 0 && $jallforms.each((function () {
            ...
            var btnformsubmit = $(this).find("[type=submit]"), btnstatus = btnformsubmit.data("form-sending-status");
            ...
            var btnformsubmit = $(this), btnstatus;
            ...
            var formtype = $activeForm.data("formactiontype"), formskey = $("#allrecords").data("tilda-formskey");
